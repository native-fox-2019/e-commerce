  ...
  <div>
    ...
    <div class="mt-5">
      <div class="row ml-auto mr-auto">
        <div class="col-4" v-for="product in $store.state.customerCart" :key="product.id">
            ...
            axios({
              ...
              data: {
                stock: (products[o].stock =
                  products[o].stock - checkout[n].amount)
              },
              headers: {
                token:
      ...
      for (let k = 0; k < checkout.length; k++) {
        for (let l = 0; l < cart.length; l++) {
          if (checkout[k].id == cart[l].id) {
            cart = cart.filter(stuff => stuff.id != stuff.id);
          }
        }
      }
